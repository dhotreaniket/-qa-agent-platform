...
def analyze_framework(test_cases: str, target_url: str, model_name: str) -> str:
    llm = ChatGoogleGenerativeAI(model=model_name, google_api_key=os.getenv("GOOGLE_API_KEY"))

    real_html = get_clean_html(target_url)
    real_html = real_html[:6000]  # trim to avoid excessive token usage
    ...
    prompt = ChatPromptTemplate.from_template(
        ...
        Recommend a Playwright (JavaScript) automation strategy in this structure:
        1. Framework: (confirm Playwright JS)
        2. Page Object Classes Needed: (with locators found directly in the HTML above)
    ...
    chain = prompt | llm
    return invoke_with_retry(chain, {"url": target_url, "html": real_html, "test_cases": test_cases})
